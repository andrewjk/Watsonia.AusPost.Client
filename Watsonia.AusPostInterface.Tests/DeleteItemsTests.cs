...
namespace Watsonia.AusPostInterface.Tests
{
	...
	public class DeleteItemsTests
	{
		...
		public async Task DeleteItems()
		{
			...
			DeleteItemsResponse updateResponse = await AusPost.DeleteItemAsync(accountNumber, username, password, createResponse.Shipments[0].ShipmentID, createResponse.Shipments[0].Items[0].ItemID);
		...
		public async Task DeleteItemsWithError()
		{
			...
			DeleteItemsResponse updateResponse = await AusPost.DeleteItemAsync(accountNumber, username, password, createResponse.Shipments[0].ShipmentID, "Invalid Item ID");
